...
def query_astock(
    ...
    Returns:
        包含 datas、code_count、chunks_info 等字段的字典

    Raises:
        AStockAPIError: API 调用失败时抛出
    """
...
环境变量:
  IWENCAI_API_KEY    API 密钥（必填，也可通过 --api-key 传入）
    ...
    parser.add_argument("--query", "-q", type=str, required=True, help="查询字符串（必填）")
    parser.add_argument("--page", type=str, default=DEFAULT_PAGE, help=f"分页参数（默认: {DEFAULT_PAGE}）")
    parser.add_argument("--limit", type=str, default=DEFAULT_LIMIT, help=f"每页条数（默认: {DEFAULT_LIMIT}）")
    parser.add_argument("--api-key", type=str, default=None, help="API 密钥（默认从环境变量 IWENCAI_API_KEY 读取）")
    parser.add_argument("--call-type", type=str, choices=["normal", "retry"], default="normal", help="调用类型: normal 或 retry（默认: normal）")
    parser.add_argument("--timeout", type=int, default=DEFAULT_TIMEOUT, help=f"请求超时时间（默认: {DEFAULT_TIMEOUT}）")
...
def main():
    ...
    try:
        ...
        if isinstance(result, dict) and "text_response" not in result:
            ...
            if has_more:
                output["pagination_tip"] = (
                    f"共查到 {code_count} 只A股，当前返回第 {args.page} 页的 {len(datas)} 条。"
        ...
        error_output = {"error": f"发生错误: {str(e)}"}
        print(json.dumps(error_output, ensure_ascii=False, indent=2))
        sys.exit(1)
