---
name: hermes-agent
description: "Configure, extend, contribute to, and optimize Hermes Agent. Covers setup, CLI reference, slash commands, gateway, security, voice, spawning, cron, kanban, skill authoring conventions, and cost/token optimization — the complete guide to working with Hermes."
version: 2.1.0
...
metadata:
  hermes:
    ...
    related_skills: [coding-agents, github, code-review-workflow, systematic-debugging]
---
...
### Post-update ImportError / stale bytecode
After `hermes update` pulls 1000+ new commits, stale `__pycache__` bytecode can cause `ImportError` (e.g., `cannot import name 'cfg_get' from 'hermes_cli.config'`). The update output may also show a merge conflict in `web/package-lock.json`.
...
# 2. If git merge conflict occurred during update, the update script
#    already resets to clean state and preserves the stash.
#    Verify with:
cd ~/.hermes/hermes-agent && git status

# 3. Restart gateway
launchctl stop ai.hermes.gateway 2>/dev/null
sleep 2
launchctl start ai.hermes.gateway 2>/dev/null

# 4. Verify fix
python3 -c "from hermes_cli.config import cfg_get; print('OK')"
```

The update auto-resolves most conflicts by resetting the working tree. If you had local modifications, they're stashed (check `git stash list`).
...
When the user asks you to create or edit a skill that ships WITH hermes-agent (not in `~/.hermes/skills/`):

### Location

`skills/<category>/<name>/SKILL.md` — use `write_file`, not `skill_manage(action='create')` (which writes to `~/.hermes/skills/`).
...
Starts with `---` at byte 0 (no leading blank line). Description ≤ 1024 chars. Total SKILL.md ≤ 100K chars; aim for 8-15K.
...
`related_skills` merges both `skills/` (in-repo) and `~/.hermes/skills/` (user-local) at load time. Prefer referencing only in-repo skills from in-repo skills.

### Editing Existing In-Repo Skills

- Small fixes: `skill_manage(action='patch')` works on in-repo skills
- Major rewrites: `write_file` the whole SKILL.md
- Supporting files: `write_file` to `skills/<category>/<name>/references/`, `templates/`, or `scripts/`
...
4. Forgetting author/license/metadata block — not enforced but expected by peers
5. Expecting the current session to see the new skill — won't appear until fresh session

---

## Cost Optimization

Load when user says "too expensive", "save tokens", "reduce cost".

### Compression (biggest lever)

```bash
hermes config set compression.threshold 0.35          # default: 0.5
hermes config set compression.target_ratio 0.15        # default: 0.2
hermes config set compression.hygiene_hard_message_limit 200
```

### Tool Output Truncation

```bash
hermes config set tool_output.max_bytes 20000          # default: 50000
hermes config set tool_output.max_lines 500            # default: 2000
```

### Disable Unused Toolsets

Each toolset adds ~150-400 tokens to system prompt. Disable unused:

```bash
hermes config set agent.disabled_toolsets '["kanban","homeassistant","feishu_doc","feishu_drive","yuanbao","rl","moa","discord_admin","spotify","video_gen","image_gen"]'
```

### Reduce Memory Size

```bash
hermes config set memory.memory_char_limit 1500        # default: 2200
hermes config set memory.user_char_limit 800           # default: 1375
```

### Cap Turns and Delegation

```bash
hermes config set agent.max_turns 50                   # default: 90
hermes config set delegation.max_iterations 25          # default: 50
```

### Verification

```bash
hermes config set compression.threshold
hermes insights --days 7
```

### Pitfalls

- Compression is lossy — threshold < 0.2 can drop rare context
- Tool output truncation can silently cut error output; bump temporarily when debugging
- Disabled toolsets require `/reset` to take effect
- Memory size below 1000 chars can force important info out
