...
> **ER diagram note:** graphify produces a **force-directed code-level knowledge graph** (classes + fields + types), NOT a traditional database ER diagram. When users ask for "ER diagram" or "entity-relationship diagram" from JPA entities:
> - Run graphify on domain/ directory for AST extraction (free, no LLM)
> - Use the detected entity/field/relationship data to construct **Mermaid `erDiagram`** (database view with columns, PK/FK) or **Mermaid `classDiagram`** (JPA view with inheritance)
> - Embed the Mermaid directly in markdown — do not serve graph.html as an ER diagram substitute
> - Mermaid erDiagram pitfall: `/` in comment strings causes parse errors — use `-` instead
