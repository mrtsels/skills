...
description: |
  ...
  scroll, drag — without stealing the user's cursor. Also includes
  macOS-native disk analysis and cleanup using Spotlight (mdfind),
  APFS tools (diskutil/tmutil), and app-specific data inspection.
version: 1.0.0
platforms: [macos]
metadata:
  hermes:
...
See `references/macos-system-settings.md` for navigating the System Settings sidebar, finding specific panes, and handling authentication dialogs. Covers: sidebar scroll pattern (Tab → Arrow Down), search field limitations, Privacy & Security sub-pane structure, Location Services location, and the Touch ID / password dialog (cannot bypass — must cancel and ask user).

## macOS Disk Management

macOS-native approach for storage analysis and cleanup. Faster than recursive directory scans because it uses Spotlight indexing and APFS introspection.
...
du -sh ~/Library/*/ | sort -rh | head -15
mdfind "kMDItemFSSize > 500000000" 2>/dev/null | grep -v "^/System/" | grep -v "^/usr/"
```

### Understanding "System Data"

macOS GUI's "System Data" = everything not classified as Apps, Documents, Photos. Main contributor is `~/Library/`. Common sources: app sandbox containers, caches, Application Support, Messages, Mail, Group Containers, Spotlight metadata.

### App-Specific Cleanup

See `references/macos-app-cleanup.md` for detailed paths and risk assessment for: WeChat, iMessage, Google Chrome (selective history/cookie), Google DriveFS, Docker, UTM VMs, AI apps (Claude Code, Codex), updater caches, Apple Intelligence caches, and Time Machine snapshots.

### Workflow

1. **Scan** — `df`, `system_profiler`, `du -sh ~/Library/*/`, `mdfind` large files
2. **Categorize** — split into safe caches vs user data (confirm)
3. **Present** — table with sizes + risk label per item
4. **Confirm** — ask user which items to delete
5. **Execute** — clean confirmed items

### Key Pitfalls

- **`du` on macOS times out** on dirs >500K files. Use `mdfind` instead.
- **Docker.raw is sparse** — `ls -l` shows virtual size; `ls -ls` shows real disk usage.
- **WeChat/QQ/Telegram Containers** — never `rm -rf` the container dir; use in-app storage settings.
- **Chrome `Default/`** — don't nuke the whole dir. Delete only `History` + `Visited Links`.
- **SIP off needed** for Time Machine snapshot deletion and some system cache operations.
...
- **Safari `type_text` writes to the wrong tab** — After clicking a tab
  label to switch tabs, the AX focus can silently stay on the previous
  tab's URL bar. When you then `type_text`, the URL gets appended to the
  wrong tab's address. **Fix**: click the address/search field (element
  with `WEB_BROWSER_ADDRESS_AND_SEARCH_FIELD` ID) of the target tab AFTER
  switching tabs, then `type_text` and press Return. Most reliable
  multi-tab workflow: Cmd+T to open a fresh tab → type URL → Return →
  Cmd+T for next site. Each new tab gives a clean address bar.
