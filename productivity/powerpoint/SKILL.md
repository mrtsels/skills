...
- **Lists:** `bullet: true` on each item, never a literal `•` (renders double bullets). Set `breakLine: true` on every array item except the last. Space bulleted paragraphs with `paraSpaceAfter`, not `lineSpacing` (huge gaps). **Text arrays MUST be `{text, options}` pairs** — pptxgenjs maps `item.text`/`item.options` (see `slideObj.text.map(item => ({text: item.text, options: item.options}))`), so flat objects like `{text: "...", bold: true}` silently collapse the whole array into ONE run-on paragraph (words join with no spaces: `"95.96If"`). Per-item `bullet.code`/`bullet.color` keys are partially ignored too: only `bullet.characterCode` (4-hex-digit) is honored, and bullet color has no OOXML emitter — colored bullets need an XML post-pass; otherwise markers render in the text color.
