...
- **ML Pipeline E2E smoke test** (`references/ml-pipeline-e2e-smoke.md`) — for verifying multi-stage pipelines (data → graph → model → eval) work together. Uses small batches of real data, runs each stage, reports integration issues. Does not expect training convergence.
- **GNN research validation** (`references/gnn-validation-patterns.md`) — pattern for fast GNN hypothesis testing by reusing existing heads and changing only the data pipeline. Covers the structural completeness validation experiment.
