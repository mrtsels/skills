...
description: Kanban multi-agent orchestration and worker runbook — decomposition playbook for orchestrator profiles, plus pitfalls, examples, and edge cases for Kanban workers. Core lifecycle auto-injected as KANBAN_GUIDANCE; this skill provides deeper detail for both orchestrators and workers.
version: 3.0.0
...
## Worker Runbook (Absorbed from kanban-worker)

This section consolidates the worker-facing guidance formerly in the separate `kanban-worker` skill. Workers are auto-loaded with the core lifecycle; this covers deeper scenarios.

### Workspace Handling

| Kind | What it is | How to work |
|---|---|---|
| `scratch` | Fresh tmp dir | Read/write freely; GC'd when task archived |
| `dir:<path>` | Shared persistent dir | Other runs will read your output |
| `worktree` | Git worktree | If `.git` doesn't exist, `git worktree add` from main repo first |

### Summary + Metadata Shapes

When calling `kanban_complete(summary=..., metadata=...)`, structure the handoff clearly:
...
**Coding task needing review (review-required pattern):**
```python
...
Good: `"Rate limit key choice: IP (NAT-unsafe) or user_id (requires auth)?"` — specific question.

### Heartbeats Worth Sending

Good: `"epoch 12/50, loss 0.31"`, `"scanned 1.2M/2.4M rows"`
Bad: `"still working"`, sub-second intervals

### Retry Scenarios

If `kanban_show` returns prior runs, check their `outcome`:
- `timed_out` — previous hit `max_runtime_seconds`. Chunk the work.
- `crashed` — OOM or segfault. Reduce memory footprint.
- `spawn_failed` — profile config issue. Ask via `kanban_block`.
- `reclaimed` — operator archived the task; stop.
- `blocked` — check the unblock comment in the thread.
...
- Claim created cards you don't have captured return values for

### Codex Lane Integration (optional)

For bounded coding tasks with clear acceptance criteria, delegate to Codex CLI. See `references/kanban-codex-lane.md` for prompt construction, monitoring, and the `kanban_complete` metadata schema. Use `templates/pmb-codex-lane-prompt.md` as the prompt template.

### Key Pitfalls

- **Task state can change between dispatch and startup** — always `kanban_show` first. If blocked/archived, stop.
- **Workspace may have stale artifacts** — read the comment thread for context.
- **Don't use CLI when tools are available** — `kanban_*` tools work across all terminal backends; `hermes kanban <verb>` may not exist in containerized backends.
